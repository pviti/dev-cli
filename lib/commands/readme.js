...
\`\`\`\n`,
        ...
        return [
            ...
            ...topics.map(t => {
                return (0, util_1.compact)([
                    ...
                    (0, util_1.template)({ config })(formatDescription(t.description)).trim().split('\n')[0],
                ]).join(' - ');
            }),
        ].join('\n').trim() + '\n';
    }
    createTopicFile(file, config, topic, commands) {
        ...
        const doc = [
            ...
            '='.repeat(bin.length),
            '',
            (0, util_1.template)({ config })(formatDescription(t.description)).trim(),
            '',
    ...
    renderCommand(config, c) {
        ...
        const title = (0, util_1.template)({ config, command: c })(formatDescription(c.description)).trim().split('\n')[0];
        const HelpClass = (0, plugin_help_1.getHelpClass)(config);
...
Readme.flags = {
    ...
    plugin: command_1.flags.boolean({ description: 'create a plugin readme doc' }),
    bin: command_1.flags.string({ description: 'optional main cli command', dependsOn: ['plugin'] })
};

...
\`\`\`\n`,
        ...
        return [
            ...
            ...topics.map(t => {
                return util_1.compact([
                    ...
                    util_1.template({ config })(formatDescription(t.description)).trim().split('\n')[0],
                ]).join(' - ');
            }),
        ].join('\n').trim() + '\n';
    }
    createTopicFile(file, config, topic, commands) {
        ...
        const doc = [
            ...
            '='.repeat(bin.length),
            '',
            util_1.template({ config })(formatDescription(t.description)).trim(),
            '',
    ...
    renderCommand(config, c) {
        ...
        const title = util_1.template({ config, command: c })(formatDescription(c.description)).trim().split('\n')[0];
